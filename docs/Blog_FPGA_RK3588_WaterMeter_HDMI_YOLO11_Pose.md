...
这种方式的好处是把人工注意力集中到最关键的误差上。比如针尖最影响角度，就单独生成 `labelme_tip_edit` 给人修；中心点由另一个人修，也能分工并行。
...
本地 CPU 可以跑脚本、检查标签、做少量验证，但不适合正式训练。YOLO11-pose 训练需要大量卷积计算，CPU 训练时间长、迭代慢；云端 GPU 可以在一两个小时内完成多轮实验，更适合调 batch、imgsz、epoch 和增强策略。

训练阶段建议关注的不只是 box mAP，还要看关键点指标和可视化结果。对于这个项目，`tip` 的准确性比框略大略小更重要，因为最终水量来自角度。

#### 为什么转 RKNN 后还要重新验证

训练得到 `.pt` 只说明模型在 PyTorch 环境下可用，不能保证 RK3588 上结果一致。导出 ONNX、转换 RKNN、量化 INT8 都可能改变数值精度和输出 layout。

因此部署前要做三组对比：

1. PyTorch `.pt` 输出是否正确。
2. ONNX 输出是否接近 PyTorch。
3. RKNN 输出是否接近 ONNX/PyTorch。

如果 FP RKNN 正常而 INT8 RKNN 针尖漂移，说明量化误差对关键点有影响，需要换量化数据集、调整混合量化策略，或者正式测量时使用 FP 模式。
