...
// All rights reserved
// File name:
// Last modified Date:     2026/04/18 16:19:14
// Last Version:           V1.0
// Descriptions:
//----------------------------------------------------------------------------------------
// Created by:             ORI2333
// Created date:           2026/04/18 16:19:14
// mail      :             ori2333.zh@gmail.com
// Version:                V1.0
// TEXT NAME:              lcd_display_test.sv
// PATH:                   fpga/sc1336_hdmi_isp/rtl/lcd_display_test.sv
// Descriptions:
//
//----------------------------------------------------------------------------------------
// source from: https://www.crazyfpga.com
//Author				:		CrazyBingo
//Technology blogs 	: 		www.crazyfpga.com
//****************************************************************************************//

module lcd_display_test#(
    parameter              [  27: 0]      DELAY_TOP                   = 74_250000            
)(
    input                               clk                        ,//system clock
    input                               rst_n                      ,//sync clock
    input                               vsync                      ,//frame sync (0=blanking)
...
//------------------------------------
//4 picture cycle cnt 鈥? switch on vsync falling edge (start of blanking)
reg             [   1: 0]      image_cnt;
reg                             switch_pending;
reg                             vsync_d1;
wire                            vsync_falling = vsync_d1 && !vsync;
...
always@(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        if (delay_0S5_flag)
            switch_pending <= 1'b1;                  // arm the switch
        if (switch_pending && vsync_falling) begin
            switch_pending <= 1'b0;                  // fire on vsync edge
